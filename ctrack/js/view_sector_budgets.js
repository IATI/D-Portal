...
view_sector_budgets.view=function()
{
	...
	ctrack.setcrumb(2);
	ctrack.change_hash();
	view_sector_budgets.ajax({year:ctrack.hash.year,funder:ctrack.hash.funder});
};

//
// Perform ajax call to get data
//
view_sector_budgets.ajax=function(args)
{
	args=args || {};
...
//		console.log("fetch sector_budgets "+year);
//		console.log(data);
		...
		for(var i=0;i<data.rows.length;i++)
		{
			...
			s.push( plate.replace("{sector_budgets_data}",d) );
		}
		...
		ctrack.chunk("sector_budgets_datas",s.join(""));
		ctrack.display();
	});
}
