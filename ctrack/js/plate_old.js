...
plate_old.fill_chunks=function(str,chunks)
{
	var hashchar="#";
	var chunks=chunks || {};
...
// this is broken :)
plate_old.out_chunks=function(chunks)
{
	var r=[];
...
plate_old.prepare=function(str)
{
...
plate_old.namespaces=[]; // array of public namespaces to lookup in

// clear namespace
plate_old.reset_namespace=function()
{
	plate_old.namespaces=[];
}

// add this dat into the namespaces that we also check when filling in chunks
plate_old.push_namespace=function(dat)
{
	if(dat)
	{
		plate_old.namespaces.push(dat);
	}
}

// remove last namespace from top of stack
plate_old.pop_namespace=function()
{
	return plate_old.namespaces.pop();
}

// lookup a str in dat or namespace
plate_old.lookup=function(str,dat)
{
	var r;
	if(dat) { r=plate_old.lookup_single(str,dat); if(r!==undefined) { return r; } } // check dat first
	for(var i=plate_old.namespaces.length-1;i>=0;i--) // last added has priority
	{ 
		r=plate_old.lookup_single(str,plate_old.namespaces[i]); if(r!==undefined) { return r; } // then look in all namespaces
	}
}
// lookup only in dat
plate_old.lookup_single=function(str,dat)
{
	...
	if(i>=0)
	{
		...
		if("object" == typeof dat[a1] ) // try a sub lookup 
		{
			...
			return plate_old.lookup_single(a2,dat[a1])
		}
	}
}

// replace once only, using dat and any added namespaces
plate_old.replace_once=function(str,dat)
{
	var aa=plate_old.prepare(str);
	...
	for(var i=0;i<aa.length;i++)
	{
		...
		if( v=="{" ) // next string should be replaced
		{
			...
			{
				var d=plate_old.lookup( v2[0],dat );
				var p=plate_old.lookup( v2[1],dat );
				if( ("object" == typeof d) && ("string" == typeof p) )
				{
					...
					if(d.length) // apply plate to all objects in array
					{
						...
						{
							dp.push( plate_old.replace_once(p,{it:d[ii],idx:ii+1}) );
						}
					}
					else // just apply plate to this object
					{
						dp.push( plate_old.replace_once(p,{it:d,idx:1}) );
					}
...
plate_old.replace=function(str,arr)
{
	var check="";
	var sanity=100;
	...
	{
		check=str;
		str=plate_old.replace_once(str,arr);
		if(--sanity<0) { break; }
	}
