...
//	var badcurl=[]
	for(var idx in downloads)
	{
		...
		if( it.url.toLowerCase().startsWith("http") || it.url.toLowerCase().startsWith("ftp") ) // some mild sanity/security
		{
			...
			curl.push("echo > logs/"+it.slug+".txt ; echo 'Downloading "+it.slug+" from \""+it.url+"\"' | tee -a logs/"+it.slug+".txt ; curl --fail --silent --show-error --retry 4 --retry-delay 10 --speed-time 30 --speed-limit 500 --insecure --ciphers 'DEFAULT:!DH' --location --output downloads/"+it.slug+".xml \""+it.url+"\" 2>&1 >/dev/null | tee -a logs/"+it.slug+".txt\n")
		}
		else
		{
